...
builder.Services.AddControllersWithViews() //framework services that enable MVC; just AddControllers if API only
    .AddJsonOptions(options => options.JsonSerializerOptions.ReferenceHandler = ReferenceHandler.IgnoreCycles); // prevents infinite loops in JSON serialization
